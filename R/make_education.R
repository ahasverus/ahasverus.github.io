make_education <- function(html) {
  ...
  for (i in 1:length(grades)) {
    ...
    if (length(yaml$"content"[[i]]$"grade"[["title"]]) > 1) {
      ...
      asset <- paste0(asset, '    <h1>', yaml$"content"[[i]]$"grade"[["title"]],
                      '</h1>\n')
    }
    ...
    # Tag NAME found
      ...
      if (length(name) > 0) {
        ...
        if (length(yaml$"content"[[i]]$"grade"$"institution"$"name") > 1) {
          ...
          for (k in langs) {
            asset <- paste0(asset, '    <span lang="', substr(k, 1, 2), '">')
            url <- which(names(yaml$"content"[[i]]$"grade"$"institution") ==
                         "url")

            # Institution URL
            if (length(url) > 0) {
              asset <- paste0(asset, '<a class="falink dotted" href="',
                              yaml$"content"[[i]]$"grade"$"institution"$"url",
                              '">')
              asset <- paste0(asset,
                yaml$"content"[[i]]$"grade"$"institution"$"name"[[k]])
        ...
        # One single language
          ...
          if (length(url) > 0) {
            ...
            asset <- paste0(asset, '<a class="falink dotted" href="',
              yaml$"content"[[i]]$"grade"$"institution"$"url", '">')
            asset <- paste0(asset,
              yaml$"content"[[i]]$"grade"$"institution"$"name")
            asset <- paste0(asset, '</a>')
      ...
      # No tag found (institution: nameof institution)
        ...
        if (length(yaml$"content"[[i]]$"grade"$"institution") == 1) {
          asset <- paste0(asset, '    <span>',
          yaml$"content"[[i]]$"grade"$"institution", '</span>\n')
        }
      }
      ...
      pos <- which(names(yaml$"content"[[i]]$"grade"$"institution") %in%
        c("city", "country", "period"))
          ...
          for (k in langs) {
            ...
            if (ncity > 0) {
              ...
              if (ncity > 1) {
                ...
                asset <- paste0(asset, '<address>',
                  yaml$"content"[[i]]$"grade"$"institution"$"city"[[k]])

              } else {

                asset <- paste0(asset, '<address>',
                  yaml$"content"[[i]]$"grade"$"institution"$"city")
              }
              ...
              if (ncountry > 0) {
                ...
                if (ncountry > 1) {
                  ...
                  asset <- paste0(asset, ', ',
                    yaml$"content"[[i]]$"grade"$"institution"$country[[k]])

                } else {
                  asset <- paste0(asset, ', ',
                    yaml$"content"[[i]]$"grade"$"institution"$"country")
                }
              }
            ...
            if (nperiod > 0) {
              ...
              if (nperiod > 1) {
                ...
                asset <- paste0(asset, '<time>',
                  yaml$"content"[[i]]$"grade"$"institution"$"period"[[k]],
                  '</time>')

              } else {

                asset <- paste0(asset, '<time>',
                  yaml$"content"[[i]]$"grade"$"institution"$"period",
                  '</time>')
              }
            }
        ...
        # One single language
          ...
          if (ncity > 0) {
            ...
            if (ncountry > 0) {
              ...
              asset <- paste0(asset, '<address>',
                yaml$content[[i]]$grade$institution$country, '</address>')
            }
          }
          ...
          if (nperiod > 0) {
            ...
            asset <- paste0(asset, '<time>',
              yaml$content[[i]]$grade$institution$period, '</time>')
          }
          asset <- paste0(asset, '</span>\n')
        }
      }
    ...
    if (length(yaml$content[[i]]$grade$thesis) > 0) {
      ...
      if (length(yaml$content[[i]]$grade$thesis) == 1) {
        ...
        asset <- paste0(asset, '  <p></em>', yaml$content[[i]]$grade$thesis,
          '</em></p>\n')
      ...
      # Multi-languages
        ...
        for (k in langs) {
          ...
          asset <- paste0(asset, '  <p lang="', substr(k, 1, 2), '"><em>',
            yaml$content[[i]]$grade$thesis[[k]], '</em></p>\n')
        }
      }
    }
