    ...
    if ((length(softs[[j]]$item$name) == 1)){
      ...
      if (length(url) > 0){
        if (j == 1){
          ...
          asset <- paste0(asset, '&nbsp;&bull;&nbsp;<a class="falink dotted" href="', softs[[j]]$item$url, '">')
          asset <- paste0(asset, softs[[j]]$item$name)
          asset <- paste0(asset, '</a>')
        }
      } else {
      ...
      for (k in langs){
        if (length(url) > 0){
          if (j == 1){
            ...
            asset <- paste0(asset, '<span lang="', substr(k, 1, 2), '">&nbsp;&bull;&nbsp;<a class="falink dotted" href="', softs[[j]]$item$url, '">')
            asset <- paste0(asset, softs[[j]]$item$name[[k]])
            asset <- paste0(asset, '</a></span>')
          }
        } else {
          if (j == 1){
            ...
            asset <- paste0(asset, '<span lang="', substr(k, 1, 2), '">&nbsp;&bull;&nbsp;')
            asset <- paste0(asset, softs[[j]]$item$name[[k]])
            asset <- paste0(asset, '</span>')
          }
        }
